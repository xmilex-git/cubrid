 ...
 *   equivalent when the heap_record_decode_fast kill-switch is ON and the
 *   record has data.  Shared entry point reused by the later index-lookup
 *   slice (R26); currently used by heap_get_mvcc_header and
 *   heap_scan_get_visible_version.
 *
 * Note: gate-free.  The kill-switch (default ON) is correctness-independent:
 * OFF falls back to the generic helper immediately.  Correctness rests on the
 * intrinsic recdes->data != NULL guard here plus or_mvcc_get_header_fast's own
 * release-active length check, not on the flag.
 */
STATIC_INLINE int
heap_or_mvcc_get_header_gated (RECDES * recdes, MVCC_REC_HEADER * mvcc_header)
{
  if (!prm_get_bool_value (PRM_ID_HEAP_RECORD_DECODE_FAST) || recdes == NULL || recdes->data == NULL)
    {
...
/*
 * Record-decode fast path (heap_record_decode_fast kill-switch, default ON).
 *
 * heap_attrinfo_recache resolves, once per cached representation, everything
 * heap_attrvalue_read re-derives per attribute on every record: the fixed-area
 * location, the bound-bit position, the disk size and the primitive type
 * handler.  heap_attrinfo_read_dbvalues then hoists the per-record header
 * arithmetic (header size, offset size, fixed-area base, bound-bit base) once
 * per record and decodes eligible fixed attributes inline; attributes that are
 * not eligible (variable, shared, class, default or deduplicate-key attributes)
 * fall back to heap_attrvalue_read within the same record pass.
 *
 * Gate-free: the fast cache is consumed ONLY when heap_attrinfo_fastdecode_usable
 * confirms it was built for the currently cached read_classrepr and its repr_id
 * still matches; otherwise the historical per-attribute loop runs.  The
 * kill-switch only governs whether the cache is built at all.
 */
...
heap_attrinfo_read_dbvalues_fast (RECDES * recdes, HEAP_CACHE_ATTRINFO * attr_info)
{
  ...
  /* Release-active safety net: parity asserts vanish under NDEBUG, so guard the
   * inline fixed reads against running past the record in all builds.  If the
   * record is shorter than the fixed area + bound bits the cached representation
   * requires, fall back to the generic per-attribute path for every value. */
  {
    int n_fixed = attr_info->read_classrepr->n_attributes - n_variable;
    int min_len = (header_size + OR_VAR_TABLE_SIZE_INTERNAL (n_variable, offset_size)
		   + fixed_length + (has_bound_bits ? OR_BOUND_BIT_BYTES (n_fixed) : 0));

    if (recdes->length < min_len)
      {
	assert (false);
	for (i = 0; i < attr_info->num_values; i++)
	  {
  ...
  for (i = 0; i < attr_info->num_values; i++)
      ...
      if (!col->fast_ok)
	{
      ...
      if (value->state != HEAP_UNINIT_ATTRVALUE
	  && (!col->skip_clear_ok || value->dbvalue.need_clear || DB_VALUE_DOMAIN_TYPE (&value->dbvalue) != col->type))
	{
	  (void) pr_clear_value (&value->dbvalue);
	}
