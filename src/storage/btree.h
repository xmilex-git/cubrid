...
struct btree_scan
{
   ...
   * BTREE_INIT_SCAN.
   *
   * The memo is a pure hint with no correctness gate: a hit is taken only when the page re-fixes with the exact
   * memoized LSA AND the lower key provably falls inside that leaf's fence range; every other outcome (and every
   * descending scan) falls back to a full root-to-leaf descent. memo_miss_streak / memo_probe_seq drive an adaptive
   * self-disable on dead-memo (write-heavy) workloads: the only state they change is whether a probe is attempted at
   * all; a miss always does the full descent, entries are never deleted on miss, and positioning never depends on
   * hit/miss. */
#define BTREE_PROBE_LEAF_MEMO_SIZE 4
  VPID memo_vpid[BTREE_PROBE_LEAF_MEMO_SIZE];	/* vpid of memoized leaf page; NULL vpid = empty entry; [0] = MRU */
  LOG_LSA memo_lsa[BTREE_PROBE_LEAF_MEMO_SIZE];	/* page LSA of memoized leaf page when it was last fixed */
  int memo_miss_streak;		/* consecutive dead-memo misses; drives the adaptive self-disable */
  unsigned int memo_probe_seq;	/* per-scan-start probe sequence; throttles re-probing while self-disabled */
...
#define BTREE_INIT_SCAN(bts)				\
  do {							\
    ...
    {							\
      int memo_i_;					\
      for (memo_i_ = 0; memo_i_ < BTREE_PROBE_LEAF_MEMO_SIZE; memo_i_++)	\
	{						\
	  VPID_SET_NULL (&(bts)->memo_vpid[memo_i_]);	\
	  LSA_SET_NULL (&(bts)->memo_lsa[memo_i_]);	\
	}						\
    }							\
    (bts)->memo_miss_streak = 0;			\
    (bts)->memo_probe_seq = 0;				\
