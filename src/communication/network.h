...
#define NET_SERVER_REQUEST_LIST \
  ...
  /* Debug only: unsafe, may break DB consistency */ \
  NET_SERVER_REQUEST_ITEM(NET_SERVER_CLEANFILEDB_DELETE_TARGET_FILE) \
  \
  /* experimental JDBC direct-server PoC */ \
  NET_SERVER_REQUEST_ITEM(NET_SERVER_JDBC_DIRECT_POC_ATTACH) \
  NET_SERVER_REQUEST_ITEM(NET_SERVER_JDBC_DIRECT_POC_EXECUTE_SELECT1)
