...
qdata_add_dbval (DB_VALUE * dbval1_p, DB_VALUE * dbval2_p, DB_VALUE * result_p, tp_domain * domain_p)
{
  ...
  /* skip no-op coerce when result NUMERIC already matches the single target domain (prec+scale); mirrors tp_value_cast_internal short-circuit */
  if (domain_p != NULL && domain_p->next == NULL
