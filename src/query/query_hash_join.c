...
qexec_hash_join (THREAD_ENTRY * thread_p, XASL_NODE * xasl, QUERY_ID query_id, VAL_DESCR * val_descr)
{
  ...
  if (xasl->proc.hashjoin.fused_state != NULL)
    {
      /* Fused probe (gated): the probe-input scan already probed the build table and
       * its gather output IS the join output -- adopt it; nothing remains to join. */
...
/*
 * Fused hash-join probe (gated, off-default; see query_hash_join.h).
 *
 ...
 * shared hash table (exactly the probe_task sharing model) and a private output sink.
 */
...
struct hjoin_fused_worker
{
  ...
  HJOIN_SINK sink;		/* bound to the worker's own gather (output) list */
...
qexec_hjoin_fused_prepare (THREAD_ENTRY * thread_p, XASL_NODE * xasl, XASL_NODE * probe_aptr, void **fused_state_out)
{
  ...
  *fused_state_out = NULL;
  ...
  /* v1 key gate: exactly matching probe/build key domains (no coercion ever applies,
   * so sharing the in-flight DB_VALUEs is byte-equivalent to reading them back from
   * a materialized probe tuple) */
  ...
  /* join output type list (outer columns from the synthesized probe type list,
   * inner columns from the complete build list; same construction as
   * hjoin_init_manager) */
  ...
  if (outer_list_id->tuple_cnt > 0)
    {
      ...
      assert (outer_list_id->type_list.type_cnt == state->output_type_list.type_cnt);
      assert (outer_list_id->last_pgptr == NULL);

      qfile_destroy_list (thread_p, xasl->list_id);
      qfile_copy_list_id (xasl->list_id, outer_list_id, false, QFILE_MOVE_DEPENDENT);

      /* the moved pages now belong to the join node; neutralize the source so the
       * probe input's own cleanup is a no-op */
      qfile_clear_list_id (outer_list_id);
    }
  else
    {
 ...
 *                                   read-only hash table (probe_task sharing model).
			       ...
			       QFILE_LIST_ID * output_list_id)
{
  ...
  /* resolve the probe key source regus: list-column positions count only the
   * NON-HIDDEN outptr entries (hidden columns are skipped by both tuple encoders) */
  worker->key_regus = (REGU_VARIABLE **) db_private_alloc (thread_p, state->key_cnt * sizeof (REGU_VARIABLE *));
  if (worker->key_regus == NULL)
    {
      goto error_exit;
    }
  memset (worker->key_regus, 0, state->key_cnt * sizeof (REGU_VARIABLE *));
  ...
  for (regu_list = outptr_list->valptrp; regu_list != NULL; regu_list = regu_list->next)
	...
	{
	  if (state->probe_value_indexes[key_index] == value_index)
	    {
	      worker->key_regus[key_index] = &regu_list->value;
	    }
	}
  ...
  for (key_index = 0; key_index < state->key_cnt; key_index++)
    {
      if (worker->key_regus[key_index] == NULL)
	{
  ...
  hjoin_sink_init_list (&worker->sink, output_list_id);
...
qexec_hjoin_fused_worker_row (THREAD_ENTRY * thread_p, void *worker_ctx, VAL_DESCR * val_descr)
{
   ...
   * owned -- exactly what data_readval (copy=false) yields on a materialized tuple,
   * since fused is gated on exactly matching key domains / no coercion) */
  for (key_index = 0; key_index < state->key_cnt; key_index++)
    {
      peek_value = NULL;
  ...
  do
      ...
      else if (need_skip_next)
	{
      ...
      error = hjoin_merge_tuple_to_list_id (thread_p, &worker->sink,
