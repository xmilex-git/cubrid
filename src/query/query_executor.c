  ...
  if (gbstate.hash_eligible && gbstate.agg_hash_context->part_list_id->tuple_cnt > 0)
    {
      ...
      /* IMP-015: close the partial list before sorting it.  While the list is
       * open for append, its writer keeps the last page fixed (last_pgptr);
       * a serial sort reads it from the same thread, but parallel sort
       * workers are different threads and dead-latch against that fix until
       * the page-latch timeout.  qfile_close_list is a no-op when the list
       * is already closed, so the existing close after the sort stays. */
      ...
      else
	{
      ...
      /* IMP-015: if this sort is running, the hash pass is over and spilled
       * partials exist, so "a live hash pass may make this sort small or
       * unnecessary" cannot apply at this call site.  Pass an explicit px arg
       * with hash_eligible = 0 so the partial-list sort is parallelism-
       * eligible (unconditional, not state-gated: in the parallel-scan merge
       * path the leader's hash state is not HS_REJECT_ALL exactly where the
       * spilled partials are largest).  POD declaration without initializer:
       * earlier gotos target labels past this point (same pattern as gby_px
       * below). */
