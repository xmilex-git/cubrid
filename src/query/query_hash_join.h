...
/* Fused hash-join probe (gated, off-default): probe pushdown into the parallel scan
 * of the probe input.  The mainblock dispatcher of a HASHJOIN_PROC arms the state
 * (qexec_hjoin_fused_prepare builds the hash table from the complete build input),
 * publishes it on the probe-input buildlist (xasl->fused_probe) and runs that aptr;
 * the parallel-scan workers then probe the shared read-only table on every in-flight
 * row and emit already-merged join tuples to their per-worker gather lists, so the
 * probe-input temp list never exists.  qexec_hash_join later just adopts the gather
 * output.  Every refusal (prepare returns a NULL state, or the scan never engages)
 * silently keeps today's materialized path. */
