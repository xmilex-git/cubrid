...
#define XASL_NL_SEMIJOIN		(0x1 << 23)	/* NL inner: emit outer once on first inner match (correlated IN/EXISTS unnest) */
#define XASL_NL_ANTIJOIN		(0x1 << 24)	/* NL inner: emit outer only when zero inner matches (correlated NOT IN/NOT EXISTS unnest) */

#define XASL_IS_FLAGED(x, f)        (((x)->flag & (int) (f)) != 0)
#define XASL_IS_NL_SEMI_OR_ANTI(x)  ((x) != NULL && ((x)->flag & (int) (XASL_NL_SEMIJOIN | XASL_NL_ANTIJOIN)) != 0)
