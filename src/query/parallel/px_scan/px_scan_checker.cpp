...
namespace parallel_scan
{
  ...
  /* Fused hash-join probe candidate marking (client side, gated on the hidden
   * kill-switch hash_join_fused_probe).
   *
   * Marks the decisively larger (probe) input buildlist of an inner hash join with
   ...
   * cardinality margin: the runtime probe role is forced from this, so a non-decisive
   * estimate marks nothing. */
  static void
  mark_fused_probe_candidate_node (XASL_NODE *hj)
  {
    ...
    /* extra (non-equi) during-join predicates keep the materialized path; the
