...
if [ "$done_flag" = "yes" ]; then
...
elif printf '%s' "$tail_area" | grep -qiE 'usage limit|limit reached|rate limit|reached your (usage|limit)|out of usage|resets? (at|in)|upgrade to (a )?(higher|paid)'; then
  state="limit_5h"
  ...
  state="error"
elif printf '%s' "$tail_area" | grep -qiE 'esc to interrupt|to interrupt\)|Thinking|Forging|Compacting|Running|tokens|Esc to'; then
  state="working"
