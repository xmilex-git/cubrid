...
name: remote-claude
description: >-
  Supervisor skill: run claude inside tmux on remote workers (cubrid@192.168.6.30/.32/.33),
  ...
  "remote claude", "토큰 소진 계정 전환 resume", "원격 감시", "ssh cubrid@192.168.6 tmux claude",
  "원격용 spec/plan 만들어".
---
...
| Item | Value |
|---|---|
| Workers | `cubrid@192.168.6.30` · `.32` · `.33` (connect by IP directly, no ssh alias). Short tag = last octet `30/32/33` |
| Readiness | `.32`/`.33` ready. `.30` setup in progress (claude/tmux/repo may be missing -> dispatch preflight blocks it) |
| Policy | **A different task per worker** (one task is not split across workers). Default account `work`. |
...
$S/rc-dispatch.sh 33 work ~/.claude/scratch/remote-claude/specs/refactor.md
$S/rc-dispatch.sh 32 work ~/.claude/scratch/remote-claude/specs/bugfix.md
# host can be the 30/32/33 short tag, or cubrid@192.168.6.NN / a raw IP
```
- Default account is `work`. If a session already exists it is refused -> to overwrite a running
  task use `--force`.
...
- **`limit_5h`**: auto-rotate `work->personal` (or back) + resume. If you just rotated and hit 5h
