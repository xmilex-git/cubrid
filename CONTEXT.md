...
| **Datum1Cache** | First-sort-key 8B inline cache in the sort entry (PG `SortTuple.datum1` analog). Fixed-length 9 types and NUMERIC precision ≤ 18 (exact int64 proxy): comparisons never touch the tuple body. Varchar / NUMERIC p19–40: `datum1` = **pointer** into sort-owned detoast/decode-once per-entry storage — the comparator reads that body once per compare (no per-compare re-decompress; no 8B content proxy in stage 1, abbreviated keys deferred to D-G5/campaign). | `p1.1-valueslot-design.md` §sort-entry (§s.2–§s.2.2) |
